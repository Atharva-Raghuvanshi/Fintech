  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-6">
        <h2 className="font-bold text-slate-800 text-lg">Unified Accounts Dashboard</h2>
            ...
            <div key={i} className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="font-semibold text-slate-900 mt-1">{acc.bank}</p>
                </div>
              ...
              <div>
                ...
                <div className="flex justify-between items-center mt-2">
                  <p className="text-xs font-mono text-slate-500">{acc.acct}</p>
                  <span className={cn("text-xs font-semibold px-2 py-0.5 rounded", acc.change.startsWith('+') ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700")}>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h3 className="font-bold text-slate-800 mb-4">Credit Utilization Overview</h3>
          <div className="flex flex-col md:flex-row gap-6 items-center">
            <div className="w-32 h-32 relative flex-shrink-0">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-xl font-bold text-slate-900">{Math.round((creditAccount.used/creditAccount.limit)*100)}%</span>
              </div>
            </div>
            <div className="flex-1 space-y-4 w-full">
              <div className="flex justify-between">
                <div>
                  <p className="text-xs text-slate-500 font-semibold uppercase">Total Limit</p>
                  <p className="font-bold text-slate-900">{formatCurrency(creditAccount.limit)}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-slate-500 font-semibold uppercase">Available</p>
                  <p className="font-bold text-emerald-600">{formatCurrency(creditAccount.available)}</p>
                </div>
              </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          <h3 className="font-bold text-slate-800 flex items-center gap-2">
            <Send className="w-5 h-5 text-indigo-600" /> Instant Money Transfers
          </h3>
          <span className="px-2 py-1 bg-emerald-100 text-emerald-700 text-[10px] font-bold rounded">UPI & IMPS</span>
        </div>
        <div className="p-6 space-y-5">
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">Send To (UPI ID / Account / Phone)</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">Pay From</label>
          ...
          <button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-lg shadow-sm transition-colors">
            Send Money Securely
          </button>
        </div>
      </div>
        ...
        <div className="p-6">
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="flex items-center justify-between p-4 rounded-xl border border-slate-100 bg-white hover:border-indigo-200 hover:shadow-sm transition-all">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className={cn("text-xs font-medium", bill.urgent ? "text-rose-500" : "text-slate-500")}>{bill.due}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-slate-900">₹{bill.amount}</p>
                  <button className="text-xs font-bold text-indigo-600 hover:text-indigo-800 mt-1 uppercase tracking-wider">Pay Now</button>
                </div>
              </div>
            ))}
          </div>
          <button className="w-full mt-4 py-3 border border-dashed border-slate-300 rounded-lg text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors">
            + Add New Biller
          </button>
        </div>
      </div>
...
function CardsTab() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="font-bold text-slate-800 text-lg">Card Control Hub</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button onClick={() => setFrozen(!frozen)} className="p-4 rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col items-center justify-center gap-2 hover:bg-slate-50 transition-colors">
            ...
            <button className="p-4 rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col items-center justify-center gap-2 hover:bg-slate-50 transition-colors">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2"><Sliders className="w-5 h-5 text-indigo-600"/> Spending Limits & Controls</h3>
            
            <div className="space-y-6">
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className="font-medium text-slate-700">Daily Transaction Limit</span>
                  <span className="font-bold text-slate-900">₹50,000 / ₹1,00,000</span>
                </div>
                <input type="range" min="0" max="100000" defaultValue="50000" className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
              </div>
              
              <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                <div>
                  <h4 className="font-medium text-slate-900">International Usage</h4>
                  <p className="text-xs text-slate-500 mt-1">Allow transactions in foreign currencies</p>
                </div>
                <button 
                  onClick={() => setIntEnabled(!intEnabled)}
                  className={cn("w-12 h-6 rounded-full transition-colors relative", intEnabled ? "bg-emerald-500" : "bg-slate-300")}
                >
                  <div className={cn("w-4 h-4 bg-white rounded-full absolute top-1 transition-transform", intEnabled ? "left-7" : "left-1")} />
                </button>
              </div>

              <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                <div>
                  <h4 className="font-medium text-slate-900">Tap to Pay (NFC)</h4>
                  <p className="text-xs text-slate-500 mt-1">Contactless payments up to ₹5,000</p>
                </div>
                <button className="w-12 h-6 rounded-full bg-emerald-500 relative transition-colors">
                  <div className="w-4 h-4 bg-white rounded-full absolute top-1 left-7 transition-transform" />
                </button>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6 lg:col-span-1">
          <div className="bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-100 rounded-xl p-6">
            ...
            <h3 className="font-bold text-slate-900">Pre-Approved Limit</h3>
            <p className="text-3xl font-bold text-emerald-700 mt-2">₹1,50,000</p>
            <p className="text-xs text-emerald-800/80 mt-2 mb-4">Based on your pristine credit health and banking history.</p>
            <button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold py-2.5 rounded-lg transition-colors">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            ...
            <p className="text-xs text-slate-500 mb-4">Convert recent transactions into easy EMIs with one click.</p>
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 bg-slate-50">
                  ...
                  <button className="text-[10px] font-bold bg-white border border-indigo-200 text-indigo-700 px-3 py-1.5 rounded hover:bg-indigo-50 transition-colors">
                    SPLIT TO EMI
                  </button>
                </div>
              ))}
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
              ...
              <div className="space-y-6">
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    ...
                    <span className="font-bold text-slate-900">{formatCurrency(emiAmount)}</span>
                  </div>
                  <input type="range" min="10000" max="500000" step="5000" value={emiAmount} onChange={(e) => setEmiAmount(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
                </div>
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <span className="font-medium text-slate-600">Duration (Months)</span>
                    <span className="font-bold text-slate-900">{emiMonths} Mos</span>
                  </div>
                  <input type="range" min="3" max="36" step="1" value={emiMonths} onChange={(e) => setEmiMonths(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
                </div>
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-slate-500">Processing Fee (1.5%)</span>
                    <span className="font-semibold text-slate-900">{formatCurrency(emiAmount * 0.015)}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-8 pt-6 border-t border-slate-100">
              <h4 className="font-semibold text-slate-800 text-sm mb-4">Flexible Repayment Controls</h4>
              <div className="flex gap-4">
                <button className="flex-1 py-2 bg-indigo-50 text-indigo-700 text-xs font-bold rounded-lg border border-indigo-100 hover:bg-indigo-100 transition-colors">
                  Setup Auto-Debit
                </button>
                ...
                <button className="flex-1 py-2 bg-rose-50 text-rose-700 text-xs font-bold rounded-lg border border-rose-100 hover:bg-rose-100 transition-colors">
                  Foreclose Loan
                </button>
              </div>
            </div>
...
function SecurityHealthTab() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-slate-900">Biometric Authentication</p>
                <p className="text-xs text-slate-500">Require FaceID/TouchID for transactions</p>
              </div>
            </div>
            <button className="w-12 h-6 rounded-full bg-emerald-500 relative transition-colors">
              <div className="w-4 h-4 bg-white rounded-full absolute top-1 left-7 transition-transform" />
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-slate-900">Hidden Fee Transparency</p>
                <p className="text-xs text-slate-500">Pre-transaction breakdown of GST & charges</p>
              </div>
            </div>
            <button className="w-12 h-6 rounded-full bg-emerald-500 relative transition-colors">
              <div className="w-4 h-4 bg-white rounded-full absolute top-1 left-7 transition-transform" />
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-slate-900">Budget Breach Alerts</p>
                <p className="text-xs text-slate-500">AI notifies when approaching limits</p>
              </div>
            </div>
            <button className="w-12 h-6 rounded-full bg-emerald-500 relative transition-colors">
              <div className="w-4 h-4 bg-white rounded-full absolute top-1 left-7 transition-transform" />
      ...
      {/* Financial Health */}
      <div className="space-y-6">
        <div className="bg-gradient-to-r from-slate-900 to-indigo-900 rounded-xl border border-slate-800 shadow-sm p-6 text-white">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
            <PieChartIcon className="w-5 h-5 text-indigo-600" /> AI Expense Analytics
          </h3>
